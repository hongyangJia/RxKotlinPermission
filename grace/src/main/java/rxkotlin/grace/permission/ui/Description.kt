...
object Description {
    ...
    fun declineDescription(message:String , mContext: Activity, permissions: ArrayList<String>): String {
        initDescription(mContext)
        var message = if (message.equals(DEFULT)) DECLINE_START else message
        val buffer = StringBuffer(message + " ")
        buffer.append(edit(mContext, permissions)).append(" " + DECLINE_END)
        return buffer.toString()
    }

    fun foreverDescription(message:String, mContext: Activity, permissions: ArrayList<String>): String {
        initDescription(mContext)
        var message = if (message.equals(DEFULT)) FOREVER_START else message
        val buffer = StringBuffer(message + " ")
